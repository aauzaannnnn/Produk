...
class produk extends Model
{
    ...
    public static function getKodeproduk()
    {
        ...
        $sql = "SELECT IFNULL(MAX(kode_produk), 'P000') as kode_produk 
                FROM produk ";
        $kodeproduk = DB::select($sql);

        // cacah hasilnya
        foreach ($kodeproduk as $kdp) {
            $kd = $kdp->kode_produk;
        }
        // Mengambil substring tiga digit akhir dari string PR-000
        $noawal = substr($kd,-3);
        $noakhir = $noawal+1; //menambahkan 1, hasilnya adalah integer cth 1
        $noakhir = 'P'.str_pad($noakhir,3,"0",STR_PAD_LEFT); //menyambung dengan string PR-001
        return $noakhir;
